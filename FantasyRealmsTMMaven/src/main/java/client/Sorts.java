...
public class Sorts {
    ...
    private static ArrayList<Malus> getDeleting(){
   ...
   //  algorithm implemented with help of: https://en.m.wikipedia.org/wiki/Topological_sorting Depth-first search
    public static ArrayList<Malus> topologicalSort(HashMap<Type, Malus> tm){
    ...
    public static ArrayList<Malus> getIndependentDeleting(HashMap<Type, Malus> tm){
        HashMap<Type, Boolean> typeisdeleted = new HashMap<>();
        ArrayList<Malus> deleting = getDeleting();
        for(HashMap.Entry<Type, Malus> entry :tm.entrySet()){
